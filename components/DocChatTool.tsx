...
import { buildDocumentIndex, searchDocumentIndex, type DocumentIndex, type DocumentPage, type PageSearchResult } from '@/lib/doc-search';

type ProcessingState = 'idle' | 'reading' | 'ready' | 'error';
...
export default function DocChatTool() {
  ...
  async function handleFileChange(file: File | undefined) {
    ...
    try {
      ...
      setIndex(buildDocumentIndex(searchablePages));
      setStatus('ready');
    } catch (cause) {
      setStatus('error');
      setIndex(null);
  ...
  function handleAsk(event: FormEvent<HTMLFormElement>) {
    ...
    const result = searchDocumentIndex(index, query);
    setAnswer(result);
    if (!result) {
        ...
        <aside className="rounded-xl border border-outline-variant bg-surface-container-lowest p-lg">
          <div className="mb-lg">
            ...
            <p className="mt-sm font-body-md text-body-md text-on-surface-variant">
              上传 PDF 后，所有解析、索引和问题匹配都在浏览器本地完成，不上传服务器。
            </p>
          </div>
          ...
          <div className="mt-lg rounded-xl bg-surface-container-low p-md font-body-md text-body-md text-on-surface-variant">
            ...
            {status === 'ready' && index && `索引完成：${index.pages.length} 页可搜索内容。`}
            {status === 'error' && error}
          </div>
