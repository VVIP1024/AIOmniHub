...
import { fetchGitHubTrends } from '@/services/github-trends';
...
const INDUSTRY_PRESETS: Record<IndustryKey, { label: string }> = {
  all: { label: '全部行业' },
  ai: { label: 'AI / 机器学习' },
  agent: { label: 'Agent / 自动化' },
  rag: { label: 'RAG / 知识库' },
  skill: { label: 'Skill / 能力编排' },
  web: { label: 'Web / 前端' },
  data: { label: 'Data / 数据工程' },
};
...
async function fetchTrendingRepositories(
  ...
  const data = await fetchGitHubTrends<GitHubRepository>({ days, minStars, limit, industry });
